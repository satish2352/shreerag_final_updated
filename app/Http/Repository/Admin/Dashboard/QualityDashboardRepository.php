...
class QualityDashboardRepository
{
    ...
    {
        // Stage 1: Material received from Security (gatepass sent to Quality, awaiting inspection)
        $GRN_genration = Gatepass::leftJoin('purchase_orders', function ($join) {
            ...
            ->where('gatepass.is_active', 1)
            ->where('gatepass.is_deleted', 0)
            ->count();

        // Stage 2: Under inspection (quality_status_id = 1133)
        $po_under_inspection = (int) DB::table('purchase_orders')
            ->where('quality_status_id', 1133)
            ->where('is_deleted', 0)
            ->count();

        // Stage 3: GRN generated / passed inspection, sent to Store (quality_status_id = 1134)
        ...
        // Stage 4: Returned to vendor / failed inspection (quality_status_id = 1144)
        $po_returned_to_vendor = (int) DB::table('purchase_orders')
            ->where('quality_status_id', 1144)
            ->where('is_deleted', 0)
            ->count();

        // Stage 5: Rejected chalans raised
        ...
        return [
            'quality_dept_counts' => [
                ...
                'po_under_inspection'        => $po_under_inspection,
                'material_need_to_sent_to_store' => $material_need_to_sent_to_store,
                'po_returned_to_vendor'      => $po_returned_to_vendor,
