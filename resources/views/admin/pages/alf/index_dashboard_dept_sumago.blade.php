...
<div class="container">
   ...
   <div class="row">
      ...
      <div class="col-md-10">
         <h5 class="card-title p-2" style="color: #696cff;">{{ $deptData->dept_name ?? '' }} - Historic SPM Analysis - {{ $triggerTime }} : {{ $start_time ?? '' }} : {{ $end_time ?? '' }} : {{ $colt_ShiftSelected ?? '' }}</h5>
      </div>

   </div>
   <div class="card h-70 shadow-lg border-0 rounded-3">
      <div class="card-body text-center p-3">
         <?php  $pd_id = $getPlantId.'_' .$getDeptId; ?>
         <form method="post" action="{{ route('/dashboardsumago') }}">
            @csrf
            <div class="row">
               <input type="hidden" name="getPlantId" id="getPlantId" value="{{ $getPlantId ?? '' }}">
               <input type="hidden" name="getDeptId" id="getDeptId" value="{{ $getDeptId ?? '' }}">
               ...
               <div class="col-md-2">
                  <select name="colt_ShiftSelected" id="colt_ShiftSelected" class="form-control">
                     <option value="">Full Shift</option>
                     @foreach($shiftsMaster as $shiftsMasterValue)
                     <option value="{{ $shiftsMasterValue->shift_id }}" {{  $shiftsMasterValue->shift_id == $colt_ShiftSelected ? 'selected' : '' }} title="{{ $shiftsMasterValue->shift_id }} : {{ $shiftsMasterValue->from_time }} to {{ $shiftsMasterValue->to_time }}">{{ $shiftsMasterValue->shift_id }}</option>
                     @endforeach
                  </select>
               </div>
               ...
               <div class="col-md-3">
                  <button type="submit" class="btn btn-primary"><i class="fa fa-search" aria-hidden="true"></i></button> 
                   <?php $pd_id = $getPlantId.'_'.$getDeptId;  ?>
                  <a href="{{ route('/dashboardsumago',['plant_id'=>$getPlantId,'dept_id'=>$getDeptId,'pd_id'=>$pd_id]) }}" class="btn btn-primary" title="SPM Analysis List" ><i class="fa fa-th" aria-hidden="true"></i></a>
             
                  {{-- <a href="{{ route('user.line_supervisor',['plant_id'=>$getPlantId,'dept_id'=>$getDeptId,'pd_id'=>$pd_id]) }}" class="btn btn-primary" title="SPM Analysis Graph" ><i class="fa fa-signal" aria-hidden="true"></i></a> --}}
               </div>
            ...
            <div class="machine_data_table">
               <div class="table-responsive">
                  <table class="table table-bordered">
                     ...
                     <tbody>
                        @if(!empty($dataFinal))
                        @foreach($dataFinal as $key=>$data)
                        
                        <tr>
                           <!-- <td class="align-middle" rowspan="{{ $data['row_span_machine'] }}">{{ $data['machine_name'] }}</td> -->

                           @if($data['row_span_machine'] !='') 

                           <td class="align-middle" rowspan="{{ $data['row_span_machine'] }}">{{ $data['machine_name'] }}</td> 

                           @endif 
                           <td> {{ $data['part_number']}} </td>
                           <td> {{ $data['actual_stoke']}} </td>
                           <td> {{ $data['run_time']}} </td>
                           <td> {{ $data['load_unload_time']}} </td>
                           <td> {{ $data['idle_time']}} </td>
                           <td> {{ $data['std_spm']}} </td>
                           <td> {{ $data['run_spm']}} </td>
                           <td> {{ $data['actual_spm']}} </td>
                           <td> {{ $data['running_variance']}} </td>
                           <td> {{ $data['avg_variance']}} </td>
...
@if($today_date11===$tdate11 && empty($colt_ShiftSelected))
<script>
   $(document).ready(function () {
   let modalOpen = false;
