...
@section('content')
    ...
    <div class="data-table-area mg-tb-15">
       ...
       <form id="filterForm" method="GET" action="{{ route('security-report-ajax') }}" target="_blank">
  ...
  <table class="table table-bordered">
    <thead>
      <tr>
                                                ...
                                                {{-- <th data-field="purchase_id" data-editable="false">Vehicle Number</th> --}}
                                                {{-- <th data-field="product_name" data-editable="false">Material Description</th> --}}
                                                {{-- <th data-field="quantity" data-editable="false">Quantity</th> --}}
                                                <th data-field="gatepass_name" data-editable="false">Gatepass Name</th>
                                                {{-- <th data-field="design_image" data-editable="false">Issued By</th> --}}
...
function fetchReport(reset = false) {
    ...
    fetch(`{{ route('security-report-ajax') }}?${params.toString()}`)
        .then(res => res.json())
        .then(res => {
 ...
 const rows = res.data.map((item, i) => {
    ...
    return `
        <tr>
            ...
            <td>${item.gatepass_name ?? '-'}</td>
            <td>${item.remark ?? '-'}</td>
            <td>
                <div style="display: flex; align-items: center;">
                    <a href="${poUrl}">
                        <button data-toggle="tooltip" title="View PO" class="pd-setting-ed">Check PO Details</button>
                    </a>
                </div>
            </td>
        </tr>
