...
@section('content')
                                ...
                                <table class="table table-bordered table-hover text-center">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th data-field="vendor_company_name" data-editable="false">Client Company Name
                                            </th>
    ...
    <script>
        ...
        function fetchReport(reset = false) {
            ...
            fetch(`{{ route('purchase-report-ajax') }}?${new URLSearchParams(formData).toString()}`)
                .then(res => res.json())
                .then(res => {
...
<td>${
    ...
    : '-'
                ...
                `).join('');
                        ...
                        if (start > 1) pagHtml +=
                            `<li><span class="page-link" onclick="goToPage(1)">1</span></li><li><span class="page-link">...</span></li>`;
                        for (let i = start; i <= end; i++) {
                            pagHtml +=
                                `<li class="${i === currentPage ? 'active' : ''}"><span class="page-link" onclick="goToPage(${i})">${i}</span></li>`;
                        }
                        if (end < totalPages) pagHtml +=
                            `<li><span class="page-link">...</span></li><li><span class="page-link" onclick="goToPage(${totalPages})">${totalPages}</span></li>`;

                        pagLinks.innerHTML = pagHtml;
                        pagInfo.innerHTML =
                            `Showing ${(res.pagination.from)} to ${(res.pagination.to)} of ${res.pagination.totalItems} rows`;
                    } else {
