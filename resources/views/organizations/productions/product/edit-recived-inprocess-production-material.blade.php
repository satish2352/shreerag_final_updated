...
<div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <!-- Your container and alert messages (if any) -->
                    ...
                    <tbody>
                        @foreach ($dataGroupedById as $key => $items)
                            @foreach ($items as $index => $item)
                                <tr>
                                    <td><span class="form-control" style="min-width:50px">{{ $loop->iteration }}</span></td>
                                    <td>
                                        @if($item->material_send_production == 0)
                                            <select class="form-control part-no" name="addmore[{{ $index }}][part_no_id]" required>
                                    ...
                                    <td>
                                        @if($item->material_send_production == 0)
                                            <input class="form-control quantity" name="addmore[{{ $index }}][quantity]" type="text" value="{{ $item->quantity }}" required>
                                        @else
                                    ...
                                    <td>
                                        @if($item->material_send_production == 0)
                                            <select class="form-control" name="addmore[{{ $index }}][unit]" required>
                                        ...
                                        @else
                                            <select class="form-control" name="addmore[{{ $index }}][unit]" disabled>
                                                <option value="">Select Unit</option>
                                                @foreach ($dataOutputUnitMaster as $unit_data)
                                                    <option value="{{ $unit_data->id }}" {{ $unit_data->id == $item->unit ? 'selected' : '' }}>
                                                        {{ $unit_data->name }}
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <input type="checkbox" name="addmore[{{ $index }}][material_send_production]" class="disabled-btn" value="1" checked disabled>
                                        @endif
                                    </td>
                                    <td>
                                        @if($item->material_send_production == 0)
