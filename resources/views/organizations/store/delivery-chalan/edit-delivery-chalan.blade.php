...
{{-- <script>
    $(document).ready(function() {
        var validator = $("#editDesignsForm").validate({
            ...
            errorPlacement: function(error, element) {
                if (element.hasClass("part_item_id") || element.hasClass("hsn_id") || element
                    .hasClass("process_id") || element.hasClass("quantity") || element.hasClass("unit_id") ||
                    element.hasClass("amount")) {
                    error.insertAfter(element);
                } else {
                    error.insertAfter(element);
        ...
        $("#add").click(function() {
            ...
            var newRow = $(`
                <tr>
                    ...
                    <td>
                        <input type="text" class="form-control hsn_name" placeholder=" " readonly />  
                        <input type="hidden" name="addmore[${i}][hsn_id]" class="form-control hsn_id" placeholder=" Amount" readonly />
                    </td>
                    <td>
                        <select class="form-control process_id mb-2" name="addmore[${i}][process_id]" id="">
            ...
            // Reinitialize validation for the new row
            initializeValidation(i);

            // Check stock when quantity changes
            $(newRow).on('keyup', '.quantity', function() {
                var partItemId = $(this).closest("tr").find('.part_item_id').val();
                var quantity = $(this).val();
                checkStock(partItemId, quantity, $(this));
            });
        });
        ...
        // Function to check stock availability
        function checkStock(partItemId, quantity, row) {
            $.ajax({
                url: '/check-stock-quantity', // Your API URL
                method: 'GET',
                data: { part_item_id: partItemId, quantity: quantity },
                success: function(response) {
                    if (response.status === 'insufficient') {
                        Swal.fire({
                            ...
                            text: 'The stock is not sufficient for this quantity.',
                        });
                        row.closest('tr').find('.quantity').addClass('is-invalid');
                    } else {
                        row.closest('tr').find('.quantity').removeClass('is-invalid');
                    }
                },
                error: function() {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Unable to check stock availability.',
                    });
                }
            });
        }
        ...
        function initializeValidation(i) {
            $('select[name="addmore[' + i + '][part_item_id]"]').rules("add", {
                required: true,
                messages: {
                    required: "Please select the Part Number",
